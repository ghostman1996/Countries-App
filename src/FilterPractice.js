...
import "../src/FilterPractice.scss";
 ...
 function search(items) {
  return items.filter((item) => {
   if (item.region == filterParam) {
    ...
    return searchParam.some((newItem) => {
     return String(item[newItem]).toLowerCase().indexOf(q.toLowerCase()) > -1;
    });
   }
  });
 }
 ...
 if (error) {
  return (
   <p>
    ...
    stopped working, but I created a simple example that demonstrate how this
    works,{" "}
   ...
   <div className='wrapper'>
    ...
    <ul className='card-grid'>
     {search(items).map((item) => (
      <li>
       <article className='card' key={item.cca2}>
